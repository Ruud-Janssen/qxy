...
GetRatings = function(diffK) {
  ...
  for (i in 1: Nt_r) {
    ...
    Bo5 = (allData$Best.of[i] == 5)
    
    rating = UpdateRating(rating, winner, loser, surface, Bo5, diffK )
  }
  ...
  for(i in (Nt_r + 1): Ntot) {
    ...
    #update rating
    rating = UpdateRating(rating, winner, loser, surface, Bo5 = Bo5, diffK = diffK )
  }
...
UpdateRating <- function(rating, winner, loser, surface, Bo5 = Bo5, diffK = diffK ) {
...
UpdateThisRatingType <- function(Ratings, games, indexWinner, indexLoser, Bo5, diffK ) {
  Kwinner = K(games[indexWinner], Bo5, diffK )
  Kloser = K(games[indexLoser], Bo5, diffK )
...
K <- function(numberOfGames, Bo5, diffK ) {
  if(Bo5 == 1){
    return(20.6 + 0.19 * diffK)
  } else {
    return((20.6 - 0.81 * diffK))
  }
