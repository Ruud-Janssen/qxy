...
total = foreach (c = 1 : 1, .combine = rbind) %do% {
  ...
  #return(foreach(wb = 1 : 15, .packages = c("leaps","bestglm", "plyr"), .combine = cbind) %dopar% {
    ...
    train_modelwithRatings = getGlickos(rdInt = initialRatingDeviance, c = rdIncrease, winbonus)
    ...
    for(q in 1 : 20) {
      ...
      RegHard = glm(y ~ 0 + glickoGamesdiff +  glickoHardGamesdiff, data = xTraincurrentHard, family = binomial)
