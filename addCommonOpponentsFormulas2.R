...
setCommonOpponentVariablesPlayerRow = 
  function(previousGames, row, wonIndexes, lostIndexes, wd, date, winner, surface) {
    if (winner == 1) {
      if(surface == 0) {
        ...
        row$Winner_COPercentPointsWon  = calculatePercentPointsWon(previousGames, wd, date, wonIndexes, lostIndexes)
        ...
        row$Winner_COPercentPointsThisSurfaceWon  = calculatePercentPointsWon(previousGames, wd, date, wonIndexes, lostIndexes)
      ...
      if (surface == 0) {
        ...
        row$Loser_COPercentPointsWon  = calculatePercentPointsWon(previousGames, wd, date, wonIndexes, lostIndexes)
        ...
        row$Loser_COPercentPointsThisSurfaceWon  = calculatePercentPointsWon(previousGames, wd, date, wonIndexes, lostIndexes)
...
calculatePercentPointsWon = function(Games, wd, date, wonIndexes, lostIndexes) {
  df = "%m/%d/%Y"
  ...
  wonDateDiff  = as.numeric(abs(as.Date(as.character(date), format = df) 
                     - as.Date(as.character(Games$Date[wonIndexes]), format = df)))
  lostDateDiff = as.numeric(abs(as.Date(as.character(date), format = df) 
                     - as.Date(as.character(Games$Date[lostIndexes]), format = df)))
  weightWon  = wd ^ wonDateDiff
  weightLost = wd ^ lostDateDiff
