...
all_atp_matches <- dplyr::bind_rows(data2000, data2001, data2002, data2003, data2004, data2005, data2006, data2007, data2008, data2009, data2010, data2011, data2012, data2013, data2014, data2015, data2016)
...
all_Sackmann_ATP_matches$atp_match <- 1
...
all_Sackmann_Challengers_matches <- bind_rows(lapply(Sackmann_challengers_files, function(x) read.table(x, header = T, sep = ",", quote = "", colClasses = "character", 
                                                                                 stringsAsFactors = FALSE, fill = TRUE)))
all_Sackmann_Challengers_matches$atp_match <- 0
...
date_format <- "%Y%m%d"
all_Sackmann_matches <- mutate(all_Sackmann_matches, tourney_date = as.Date(as.character(tourney_date), format = date_format ))
all_Sackmann_matches <- all_Sackmann_matches[with(all_Sackmann_matches, order(tourney_date, atp_match)), ]

all_Sackmann_matches <- all_Sackmann_matches[all_Sackmann_matches$tourney_date >= as.Date("20000101", format = date_format), ]

saveDatasets(all_Sackmann_matches, dir_result_datasets, "all_Sackmann_matches", "lvl1")